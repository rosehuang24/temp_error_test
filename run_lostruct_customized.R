...
for (k in seq_along(bcf.files)) {
    ...
    for (chrom.index in seq_along(file.positions)) {
        ...
        pca.file <- file.path( opt$outdir, paste0(chrom.name,"withNA.pca.csv") )
        regions.file <- file.path( opt$outdir, paste0(chrom.name,"withNA.regions.csv") )
        ...
        all.r.p.noNA <- na.omit(cbind(all.regions,all.pcas)) #rm windows with NAs  in the matrix. It worked for me because the windows either have true values or all NAs
        all.r.p.noNA.no0 <- na.omit(all.r.p.noNA[apply(all.r.p.noNA, 1, function(row) all(row !=0 )), ])#this step removes 0s as it will create NA in MDS calculation
        all.pca.noNA.no0<- na.omit(all.pcas[apply(all.pcas, 1, function(row) all(row !=0 )), ])

        noNA_region.pca.file <- file.path( opt$outdir, paste0(chrom.name,"noNA_No0_region.pca.csv") )
        write.csv( all.r.p.noNA.no0, file=noNA_region.pca.file, row.names=FALSE ) #simple create a midfile references for the "Okay" windows in case i need to look back and find those in the chromosomes.
...
system.time( pc.distmat <- pc_dist(  all.pca.noNA.no0, npc=opt$npc ) )
...
mds.coords <- cbind( data.frame(
                        ...
                        cmdscale( pc.distmat, k=opt$nmds )
                    )
